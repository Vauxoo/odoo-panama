        ...
        if city_id:
            ...
            return {'value': {'city': city.name,
                    'state_id': city.state_id.id,
                              'country_id':
                              city.country_id and city.country_id.id or False
                              }
                    }
        return {}

    def onchange_state_city(self,
                            cr, uid, ids, state_id, city_id, context=None):
        res = super(ResPartner, self).onchange_state(
            cr, uid, ids, state_id, context)
        if city_id and state_id and self.pool.get('res.country.state.city') \
                .browse(cr, uid, city_id, context).state_id.id != state_id:
